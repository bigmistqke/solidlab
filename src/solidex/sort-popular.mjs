...
const utilities = [
  ...
  },
  {
    ...
    author_url: 'https://gitlab.com/olgam4',
...
// Function to fetch downloads for a given package
...
// Function to search for a package if downloads are 0
async function searchPackage(packageName) {
  const url = `https://registry.npmjs.org/-/v1/search?text=${packageName}&size=1`
  try {
    const response = await fetch(url)
    const data = await response.json()
    const name = data.objects[0]?.package.name || ''
    console.log('kobalte', name, packageName)
    return name
  } catch (error) {
    console.error(`Failed to search for package ${packageName}:`, error)
    return ''
  }
}

// Main function to fetch and sort by downloads
async function fetchAndSortByDownloads() {
  const promises = utilities.map(async utility => {
    const packageName = utility.title /* link.split('/').pop() // Extract package name from URL */
    let downloads = await fetchDownloads(packageName)
    let queriedPackage = undefined

    // If downloads are 0, try to find the correct package
    if (downloads === 0) {
      const correctPackageName = await searchPackage(packageName)
      if (correctPackageName) {
        downloads = await fetchDownloads(correctPackageName)
        queriedPackage = correctPackageName // Store the corrected package name if found
      }
    }

    return { ...utility, downloads, queriedPackage }
  })
